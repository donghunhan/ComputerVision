...
for i in range(roi.shape[0]):
    for j in range(roi.shape[1]):
        ...
        if ((roi[i,j,0] +50) >255)  :
            negative_img[i,j,0]=255
        else :
            negative_img[i,j,0] = roi[i,j,0] +50 # b 채널의 픽셀 계산
        
        if ((roi[i,j,1] +50) >255)  :
            negative_img[i,j,1]=255
        else :
            negative_img[i,j,1] = roi[i,j,1] +50 # g 채널의 픽셀 계산
        
        if ((roi[i,j,2] +50) >255)  :
            negative_img[i,j,2]=255
        else :
            negative_img[i,j,2] = roi[i,j,2] +50 # r 채널의 픽셀 계산
